...
```text
用户问题 → 三路元数据召回 → 最小上下文构建 → SQL 生成 → 安全校验 → SQLite 执行 → 带来源的结果
```

这个版本刻意使用 SQLite 和内置样例数据，以便无需 Docker 就能先跑通与测试。元数据召回按指标、字段和字段取值三路进行；当前是别名和值的本地精确匹配，接口边界已预留，可进一步替换为向量库与全文检索。
...
- 当前召回是可测试的词法原型，不是语义向量检索；同义词需先写入别名，下一阶段将接入 Embedding/Qdrant。
